...
#%%-create model
model=ARIMA(df_train, order=(2,1,1), trend='t')
fitted=model.fit()
...
#%%
fc=fitted.get_forecast(len(df_test))
fc_value=fc.predicted_mean
fc_value.index=df_test.index
conf=fc.conf_int(alpha=0.05) #95% conf
lower_series=conf['lower Close']
lower_series.index=df_test.index
upper_series=conf['upper Close']
upper_series.index=df_test.index
